...
const Trust = () => {
    const points = [
        ...
        {
            icon: <TrendingUp className="w-8 h-8 text-[#3b82f6]" />,
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {points.map((item, index) => (
                        <motion.div
                            ...
                            className="rounded-3xl p-8 card-hover-premium group"
                            style={{ background: '#060810', border: '1px solid rgba(59,130,246,0.12)' }}
                        >
                            <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300" style={{ background: 'rgba(37,99,235,0.1)', border: '1px solid rgba(59,130,246,0.2)' }}>
                                {item.icon}
                            </div>
                            <h3 className="text-2xl font-bold mb-3 text-white">{item.title}</h3>
                            <p className="text-[#94a3b8] leading-relaxed text-lg">
                                {item.description.includes("7–14 days") ? (
                                    <>Most projects launch within <span className="font-bold text-white">7–14 days</span> without compromising quality.</>
                                ) : (
                                    item.description
                                )}
                            </p>
